    ...
    <div class="flex items-center space-x-5">
      <div class="flex items-center space-x-4 text-gray-600">
        ...
        <button class="relative hover:text-[#ef4444] transition">
          ...
          <span class="absolute -top-1.5 -right-1.5 bg-[#ef4444] text-white text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full border border-white">0</span>
        </button>
        ...
        <a href="<?= BASEURL; ?>/cart" class="relative hover:text-[#ef4444] transition">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
          </svg>

          <span id="nav-cart-badge" class="<?= ($cart_count <= 0) ? 'hidden' : 'flex' ?> absolute -top-1.5 -right-1.5 bg-[#ef4444] text-white text-[9px] font-bold w-4 h-4 items-center justify-center rounded-full border border-white transition-transform duration-300">
            <?= $cart_count; ?>
          </span>
        </a>
      ...
      <?php if (isset($_SESSION['user_id'])): ?>
        ...
        <div class="relative group">
          <button class="flex items-center space-x-2.5 text-gray-700 hover:text-[#ef4444] transition-colors focus:outline-none py-1">
            <div class="w-9 h-9 bg-red-50 text-[#ef4444] rounded-full flex items-center justify-center font-bold text-sm border border-red-100 shadow-sm">
              <?= strtoupper(substr($_SESSION['name'] ?? 'U', 0, 1)); ?>
            </div>
            <div class="text-left hidden lg:block">
              <p class="text-xs font-bold text-gray-900 leading-tight max-w-[100px] truncate">
                <?= $_SESSION['name'] ?? 'User'; ?>
              </p>
              <p class="text-[10px] text-gray-500 font-medium capitalize">
                <?= str_replace('_', ' ', $_SESSION['role'] ?? 'pembeli'); ?>
              </p>
            </div>
            <svg class="w-4 h-4 text-gray-400 group-hover:text-[#ef4444] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>

          <div class="absolute right-0 mt-1 w-56 bg-white rounded-xl shadow-lg border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
            <div class="p-2">
              <?php
              $dashboard_link = '/home';
              ...
              ?>
              <a href="<?= BASEURL . $dashboard_link; ?>" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#ef4444] rounded-lg transition-colors font-medium">
                Dashboard
              </a>
              <a href="<?= BASEURL; ?>/pesanan" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#ef4444] rounded-lg transition-colors font-medium">
                Pesanan Saya
              </a>
              <div class="border-t border-gray-100 my-1"></div>
              <a href="<?= BASEURL; ?>/auth/logout" class="flex items-center px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors font-bold">
                Keluar
              </a>
            </div>
          </div>
